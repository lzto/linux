 ...
 * TODO:
 *  associate aux data with thread
					 ...
					 struct pebs_record_skl *pebs_record)
{
	...
	sample.ip = le64_to_cpu(pebs_record->ip);
	sample.pid = pebsq->pid;
	sample.tid = pebsq->tid;
	sample.addr = le64_to_cpu(pebs_record->real_ip);
	//sample.id = pebsq->pebs->branches_id;
	//sample.stream_id = pebsq->pebs->branches_id;
	sample.period = 1;
	sample.cpu = pebsq->cpu;
#if 0
	//FIXME! need inject???
	if (pebs->synth_opts.inject) {
		event.sample.header.size = pebs->branches_event_size;
		ret = perf_event__synthesize_sample(&event,
						    pebs->branches_sample_type,
						    0, &sample,
	...
	if (ret)
		pr_err("Intel PEBS: failed to deliver branch event, error %d\n",
		       ret);
